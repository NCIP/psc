class CopyActivtyIdsAndDetails extends edu.northwestern.bioinformatics.bering.Migration {
    void up() {     
        if (databaseMatches('postgresql')) {
            ...
            execute('UPDATE scheduled_events SET activity_id=(SELECT p.activity_id FROM planned_events p INNER JOIN scheduled_events s ON s.planned_event_id=p.id where scheduled_events.id=s.id)')
            execute('UPDATE scheduled_events SET details=(SELECT p.details FROM planned_events p INNER JOIN scheduled_events s ON s.planned_event_id=p.id where scheduled_events.id=s.id)')
        }
    }
