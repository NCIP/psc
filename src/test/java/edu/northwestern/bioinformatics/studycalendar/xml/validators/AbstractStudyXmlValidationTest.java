...
import edu.northwestern.bioinformatics.studycalendar.xml.writers.*;
 ...
 */
public class AbstractStudyXmlValidationTest extends StudyCalendarXmlTestCase {
    ...
    protected Amendment firstExpectedAmendment;
    private Amendment developmentAmendment;
    protected Element eFirstAmendment;
    protected Element eDevelopmentAmendment;
    protected Element eCalendar;
    private Amendment secondAmendment;
    protected Amendment secondExpectedAmendment;
    protected Element eSecondAmendment;
    ...
    private StudySegmentDeltaXmlSerializer studySegmentDeltaXmlSerializer;
    protected Add add3;
    ...
    protected void setUp() throws Exception {
        ...
        firstExpectedAmendment = Fixtures.createAmendment("[First Expected]", edu.nwu.bioinformatics.commons.DateUtils.createDate(2008, Calendar.APRIL, 16), true);
        secondAmendment = Fixtures.createAmendment("[Second]", edu.nwu.bioinformatics.commons.DateUtils.createDate(2008, Calendar.JANUARY, 11), true);

        secondExpectedAmendment = Fixtures.createAmendment("[Second Expected]", edu.nwu.bioinformatics.commons.DateUtils.createDate(2008, Calendar.AUGUST, 16), true);


        expectedDevelopmentAmendment = edu.northwestern.bioinformatics.studycalendar.domain.Fixtures.createAmendment("[Expected Development]", edu.nwu.bioinformatics.commons.DateUtils.createDate(2008, Calendar.FEBRUARY, 13), true);

        developmentAmendment = edu.northwestern.bioinformatics.studycalendar.domain.Fixtures.createAmendment("[First Expected]", edu.nwu.bioinformatics.commons.DateUtils.createDate(2008, Calendar.APRIL, 16), true);
        ...
        secondExpectedAmendment.addDelta(plannedCalendarDelta);
        secondExpectedAmendment.addDelta(periodDelta);
        secondExpectedAmendment.addDelta(studySegmentDelta);
        secondExpectedAmendment.addDelta(epochDelta);
...
//                amendmentSerializer.setStudy(study);
    ...
    protected void readXml() {
        importTemplateService.readAndSaveTemplate(null, validStudyXml);
    }

    protected void expectAndResolveStudy() {
        study.setAmendment(secondExpectedAmendment);
        study.pushAmendment(firstExpectedAmendment);
        expectResolveStudy("Test4", study);
    }

    protected void expectResolveStudy(String name, Study resolved) {
        expect(studyService.getStudyByAssignedIdentifier(name)).andReturn(resolved);
