...
public class SpringBeanConfigurationTools {
     ...
     */
    public static <T> T makeAware(ApplicationContext parent, T bean) {
        if (bean instanceof BeanFactoryAware) {
            ((BeanFactoryAware) bean).setBeanFactory(parent);
        }
        ApplicationContextAwareProcessor processor = new ApplicationContextAwareProcessor(parent);
