class UpdateCsmUserNameAndEndDate extends edu.northwestern.bioinformatics.bering.Migration {
    void up() {
        if (databaseMatches('hsqldb') || databaseMatches('oracle')) {
            ...
            if (databaseMatches('hsqldb')) {
                execute("UPDATE csm_user cu SET end_date = CURRENT_TIMESTAMP WHERE user_id in (SELECT csm_user_id FROM users WHERE active_flag = 't')");
            } else {
                execute("UPDATE csm_user cu SET end_date = CURRENT_TIMESTAMP WHERE user_id in (SELECT csm_user_id FROM users WHERE active_flag = 1)");
            }
        } else {
    ...
    void down() {
        execute("update csm_user set first_name='.' where first_name != '.'")
        execute("update csm_user set last_name='.' where last_name != '.'")
        execute("update csm_user set end_date=null where end_date is not null")
    }
}